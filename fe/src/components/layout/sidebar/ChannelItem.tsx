...
import { FindRecommendedChannelsQuery } from "@/graphql/gennerated/output"
...
const ChannelItem = ({ channel }: ChannelItemProps) => {
    ...
    return (
        ...
        isCollapsed ?
            ...
            :
            (
                <Button className={cn('mt-1.5 h-11  w-full justify-start', isActive && 'bg-accent')} variant='ghost' asChild>

                    <Link href={`/${channel.username}`} className="flex w-full  items-center ">

                        <ChannelAvatar channel={channel} size='default' isLive={channel.stream.isLive} />
